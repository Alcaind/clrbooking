...
$app->get('/rooms', function (Request $request, Response $response) {
    ...
    $rooms = \App\Models\Rooms::with(['room_category:id,descr', 'config', 'room_use', 'tms', 'items', 'users'])->get();
...
$app->get('/rooms/{id}', function (Request $request, Response $response, $args) {
    ...
    $id = $args['id'];
    try {
        $room = \App\Models\Rooms::with(['room_category:id,descr', 'config', 'room_use', 'tms', 'items', 'users'])->find($id);
...
$app->get('/active/rooms', function (Request $request, Response $response, $args) {
    header("Content-Type: application/json");
    try {
        $rooms = \App\Models\Rooms::with(['room_category:id,descr', 'config', 'room_use', 'tms', 'items'])->where('status', '=', 1)->get();
    } catch (PDOException $e) {
        $nr = $response->withStatus(404);
        $error = new ApiError();
        $error->setData($e->getCode(), $e->getMessage());
